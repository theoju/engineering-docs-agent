...
The schema is defined in `agents/schemas/source_collector.schema.json` and mirrored verbatim in the `## Output schema (canonical)` block of `agents/source-collector.md`. Both copies are enforced to stay identical by `tests/agents/test_schema_md_sync.py:30`.
...
CCE-10 closes this by setting `CLAUDE_STOP_VERIFY=0` in the subprocess environment. The relevant line is `scripts/orchestrator_runner.py:419`:

```python
run_kwargs["env"] = {**os.environ, "CLAUDE_STOP_VERIFY": "0"}
```

The env dict extends `os.environ` rather than replacing it, so PATH, credentials, and all other ambient variables still reach the child process. `tests/orchestrator/test_dispatch_subagent_env.py:21` enforces both invariants: `CLAUDE_STOP_VERIFY` must be `"0"`, and `PATH` must match the parent's `os.environ["PATH"]`.

## Defense-in-depth: `_rescue_json_object`

CCE-10 closes the stop-verify pathway, but other contamination patterns exist (CCE-15 documented an `"★ Insight"` preamble injected by an explanatory-output-style plugin). The orchestrator also carries a prose-tolerant fallback at `scripts/orchestrator_runner.py:128`:
...
Every agent's `.md` file must embed its output schema in a `## Output schema (canonical)` fenced JSON block, and that block must be byte-for-byte equivalent to the corresponding `.schema.json` file. The test at `tests/agents/test_schema_md_sync.py:30` checks all seven agents. If you update `agents/schemas/source_collector.schema.json`, also update the inline block in `agents/source-collector.md`, or the test fails.
...
The orchestrator clips out-of-window PRs at `scripts/orchestrator_runner.py:273` (`_clip_prs_to_window`) as a safety net, but the contract requires the agent to clip first.
