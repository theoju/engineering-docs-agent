...
`tests/state_io/test_add_partial_blind.py` covers the `add_partial` precedence table directly: a blocking reason with no kwargs sets both `partial` and `blind` and appends to `blind_reasons`; `degraded=True` sets `partial` only; `info_only=True` touches neither. `tests/orchestrator/test_classification_coverage.py` enumerates every blocking `add_partial` call site in `scripts/orchestrator_runner.py` and fails if one is left unclassified — this is what keeps the audit from decaying as the file grows. `tests/orchestrator/test_dispatch_reasons_classification.py` covers the seven `_record_dispatch_reasons` callsites, and `tests/orchestrator/test_schema_invalid_soft_fail.py` pins the three `schema_invalid:` call sites that carry different classifications by call site rather than by reason string. `tests/orchestrator/test_blind_run_interlocks.py` is the regression test for the auto-merge gap this change closes: blind and `advance_cursor_backed=True` must still skip. `tests/orchestrator/test_cursor_backed_merge.py` and `tests/orchestrator/test_deferral_skip.py` cover the surrounding CCE-140 cursor-walk and skip machinery that CCE-151 later extended, and `tests/orchestrator/test_state_advancement_invariant.py` pins the watermark-advance contract across both.
...
- Tests: `tests/state_io/test_add_partial_blind.py`, `tests/orchestrator/test_classification_coverage.py`, `tests/orchestrator/test_dispatch_reasons_classification.py`, `tests/orchestrator/test_schema_invalid_soft_fail.py`, `tests/orchestrator/test_blind_run_interlocks.py`, `tests/orchestrator/test_cursor_backed_merge.py`, `tests/orchestrator/test_deferral_skip.py`, `tests/orchestrator/test_state_advancement_invariant.py`, `tests/orchestrator/test_pipeline_integration.py`
