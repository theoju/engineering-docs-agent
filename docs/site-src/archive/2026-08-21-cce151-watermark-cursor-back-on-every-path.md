...
4. **Reason strings are cause-dependent, deliberately — with one un-renamed exception worth knowing before you grep.** The walk now serves two causes — time-budget truncation and held-back PRs — so most of the reason strings switch on a local `_rsn = "time_budget" if time_truncated else "held_back"` inside `run`: `{_rsn}_no_advance_no_cursor`, `{_rsn}_no_advance_unanchored_deferred`, and `{_rsn}_advance_out_of_window` (the "cursor unresolvable in repo" case) all correctly read `held_back_*` on a non-truncated held-back run. One sibling did not get the same treatment: the branch that fires when `_sha_in_window` rejects an otherwise-resolvable cursor still emits the literal `time_budget_advance_out_of_window`, unconditionally, even when the run was never time-truncated. It is reachable on the held-back path exactly like its `_rsn`-prefixed neighbors are — it just wasn't updated to match. If you are triaging a held-back (non-time-truncated) run and grep the digest for `held_back_advance_out_of_window` and find nothing, check for the literal `time_budget_advance_out_of_window` string too before concluding the cursor advanced cleanly. The `time_budget_*` family that IS meant to stay fixed stays byte-identical to its pre-CCE-151 text: `tests/orchestrator/test_time_budget.py` and the deferral-skip tests assert those exact strings, and existing runbooks tell operators to grep for them — this stray literal happens to satisfy that assertion too, which is likely why it slipped past review. Neither family was added to `_MERGE_VETO_REASON_PREFIXES` (still the single `app_token_unavailable` entry), and both sites keep `degraded=True`, so CCE-144's blind/degraded classification is unaffected by this change either way.
...
If instead the baseline correctly held but you're trying to explain why via `partial_reasons`, don't stop at `held_back_advance_out_of_window` — see Trap 4 above for the one sibling reason that still reads `time_budget_advance_out_of_window` on a run that was never time-truncated.
