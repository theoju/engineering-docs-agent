...
The third reason is the one a stock host actually meets: `DEFAULT_TIME_BUDGET_SECONDS` (2700) plus the default 900-second merge poll already fills the whole 3600-second token with nothing left for the 285-second tail, so the resolver has no overrun to grant. That is not a config error — the budget can be perfectly serviceable on its own, it just leaves no room on top — so the resolver holds the cap flat at the budget, records an advisory `authoring_hard_cap_squeezed` reason, and degrades to the pre-CCE-152 cut: never worse, never silent. The wording is deliberately not "hard cap 2700s over budget 2700s" — a number over itself would hide the reason the overrun is missing. A cap that is merely narrowed by the clamp, rather than squeezed flat, gets a third advisory of its own, `authoring_hard_cap_clamped`, naming the value asked for, the ceiling it was cut to, and which config key (or the `* 1.15` default) produced it — an operator's next move there is arithmetic, and the reason has to carry the numbers to do it with.

An explicit `authoring_hard_cap_seconds` at or below the resolved budget is rejected as a config error (exit 2, logged as `run.authoring_hard_cap_seconds (1800)` alongside the resolved budget) rather than silently clamped up — equal collapses the hard deadline onto the soft one and quietly restores the pre-fix mid-group cut in exactly the place an operator was trying to configure it away. The key itself needed a schema fix to be usable at all: `run` in `templates/config.schema.json` sets `additionalProperties: false`, and `authoring_hard_cap_seconds` was documented and read by the resolver before it was declared there — so a host that followed the documentation and set it exited 2 at config load, every night, before authoring even started. It is now declared (`integer`, `minimum: 1`) alongside the rejection behavior it can't itself express — JSON Schema has no way to compare two sibling properties, so the "must exceed `time_budget_seconds`" rule stays enforced by the resolver, not the schema.

## Verification

`tests/orchestrator/test_pr_boundary_authoring_cut.py` drives the fix end to end: a past-soft-deadline cut mid-group keeps running to the PR boundary and the baseline advances to it; a shared batch between two PRs is recognized as owned by the older one; a hard-cap cut still lands inside the group and reports that the baseline cannot advance; a skipped (`unknown_lens`) batch doesn't fabricate a spurious boundary; an explicit `authoring_hard_cap_seconds` at or below budget is rejected with exit 2 rather than a traceback; a squeezed host (the stock 2700s default) records `authoring_hard_cap_squeezed` without flipping `partial`, and its cut reason reads "hard cap held at budget ... by the App-token TTL" rather than a number over itself; and the clamp reaches the runtime deadline the cut actually checks against, not just the resolver's return value. `tests/orchestrator/test_authoring_hard_cap_bounds.py` covers the cap resolver's clamping and squeeze arithmetic in isolation.
