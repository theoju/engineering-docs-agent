...
`NO_COLOR=1` does **not** fix this — node gives `FORCE_COLOR` precedence over it — so the obvious-looking patch would have passed review and changed nothing.

## The fix

Pin the reporter explicitly rather than rely on node's default selection: `node --test --test-reporter=tap`. TAP's summary lines are a machine-readable contract (`# pass 53`, `# fail 0`), not human-facing prose, so they carry no colour regardless of `FORCE_COLOR`. `tests/templates/test_sdd_fidelity_gate_node.py` now builds its `node --test` invocation with `--test-reporter=tap` (`_NODE_TEST_ARGV`) and matches the summary with `_PASS_RE`/`_FAIL_RE`, anchored to the `#` prefix instead of the reporter-dependent `[ℹ#]` alternation TAP's own format already satisfied.

The suite also gained a regression test, `test_the_gate_survives_an_environment_that_forces_colour`, that sets `FORCE_COLOR` explicitly rather than relying on session inheritance — the pre-existing `test_sdd_fidelity_gate_js_suite_passes` alone only exercises the coloured path when the person running it happens to be in an agent session, and passes vacuously everywhere else. Both are skipped, not failed, when `node` isn't installed (`_SKIP_WITHOUT_NODE`), consistent with the plugin's degrade-gracefully mandate — a host without Node still gets a green Python suite. The wrapper's summary-parsing assertion also `repr()`s an unparseable `stdout` rather than printing it raw, since an unparseable summary usually carries control characters a terminal renders invisibly; a raw print would have sent the reader chasing their own diff instead of the bytes. A floor of `_MIN_EXPECTED_TESTS = 20` on the parsed pass count guards against a gutted or all-skipped suite (which still exits 0 and prints `pass 0`) reading as green.
...
The fix's own scope was established, not assumed: `CHANGELOG.md` records that all of this repo's other stdout-parsing sites in `scripts/` shell out to `git` (`rev-parse`, `ls-files`, `ls-remote`, `remote get-url`, `branch --show-current`) or read `gh --json` output, and `git` ignores `FORCE_COLOR` entirely — that's a Node-ecosystem convention, not a universal one — defaulting `color.ui=auto` (off when stdout isn't a TTY). So this was the one site actually exposed.
