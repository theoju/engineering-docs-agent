---
title: API reference
status: draft
---

# API reference

_This section is scaffolded. Content will be added here._

<!-- docs-agent:overview:start -->
**Components**

- **Orchestrator** — 4 modules
- **Generators** — 7 modules
- **Lint** — 17 modules
- **Setup** — 5 modules
- **Verification** — 3 modules
- **Integrations** — 3 modules
- **Other** — 3 modules

**Contracts**

- [content-validator output](contracts/content_validator.schema.md)
- [gap-detector output](contracts/gap_detector.schema.md)
- [notifier output](contracts/notifier.schema.md)
- [page-author output](contracts/page_author.schema.md)
- [pr-summarizer output](contracts/pr_summarizer.schema.md)
- [publish-verifier output](contracts/publish_verifier.schema.md)
- [source-collector output](contracts/source_collector.schema.md)
<!-- docs-agent:overview:end -->
