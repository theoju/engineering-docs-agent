...
**PR summaries are cached across runs (CCE-159).** The collection window is a lookback from a baseline that advances slowly, so the same merged PRs are re-collected night after night until the cursor passes them — and each re-collection used to re-dispatch `pr-summarizer` against a merge commit that cannot have changed. On the `advanced-data-import-system` host that was 52 of 58 PRs on 2026-08-17, about 1.53M tokens a night. A summary is now stored in `state.json` under `pr_summaries`, keyed `{owner}/{name}#{pr}`, and reused on the next run — but only while **both** the PR's `merge_sha` and a fingerprint of `agents/pr-summarizer.md` still match what was stored. A re-merge or a rewritten history changes the first; editing the summarizer changes the second; either one re-buys that summary. Entries are evicted 30 days after they were last seen in a window, not when they fall out of one, so a transiently short window (a degraded source-collector) does not wipe the cache and a repeatedly-deferred PR does not age out of it. The reuse count rides the digest as an advisory `pr_summaries_reused: n/m` — it reports work the run did not have to do, so it never marks a run partial. Set `run.reuse_pr_summaries: false` to re-summarize every PR every run.

**The review window is capped at 10 PRs per run (CCE-169), and this is ON by default.** Every existing host starts capping on upgrade with no config edit; set `run.window_pr_cap` to raise it, or `0` to restore the pre-CCE-169 unbounded window. The window had no upper bound, so a stalled baseline was self-reinforcing: it widened by a day every night, each run finished a smaller fraction of it, and the only pre-existing truncation was the time-based cut _inside_ the admission loop, which fires after the run has already begun failing to keep up. The cap fires **before** admission, oldest-first, so the work per run is bounded rather than the clock. PRs beyond the cap are a third category, not a deferral: they stop the CCE-109 cursor at the cap boundary so the baseline never walks past them, but they accrue **no** deferral count and are never exposed to the `run.deferral_skip_threshold` hatch — the run did not attempt them, so it must not abandon them. A capped run is partial with a `held_back_window_capped: n of m PRs held for a later run (cap c)` reason and still auto-merges, which is what lets the next run take the next `c` PRs. One exemption: a PR the source-collector returned **without** a `merge_sha` is admitted even past the cap, because the cap may only hold back a PR a later window can re-anchor — such a PR sorts last and would otherwise land in the capped tail, be held out of the cursor, and be stranded behind a baseline that advanced past its real merge commit. So the work bound is `cap + unanchored`, not exactly `cap`.
...
Part of that sizing is now **enforced structurally**, not just documented: `resolve_authoring_hard_cap` clamps the hard cap down to `GITHUB_APP_TOKEN_TTL_SECONDS - checks_timeout_seconds - 285s` (the poll term is dropped for a `merge.policy: manual` host, which never runs it), so no host can configure an authoring **overrun** that outlives its own token **measured from the moment the orchestrator starts**. Read "overrun" strictly: the clamp bounds the stretch _above_ `time_budget_seconds`, and it applies on the two paths where there is one to bound — a cap that already fits under the ceiling, and a cap narrowed down to it. **It does not bound the budget.** A host whose ceiling lands at or below its own budget gets no ceiling applied at all (the second bullet below), and is bounded by `time_budget_seconds` alone; `time_budget_seconds + checks_timeout_seconds` can reach the token's TTL exactly, and exceed it as the budget grows. Note also what the enforced terms cover: they are the ones `run()` can see — its own budget, its own overrun, the poll it will run, and the tail reserve — all counted from the `clock()` call at `run()` entry. The workflow mints the token in the job's FIRST step, several minutes of checkout and install earlier, and nothing in the process can measure that gap. **Sizing it is yours**: subtract your job's setup time from the hour before you spend the rest, and keep `time_budget_seconds + the overrun (authoring_hard_cap_seconds - time_budget_seconds) + checks_timeout_seconds + 285` — that is, `authoring_hard_cap_seconds + checks_timeout_seconds + 285` — under what is left. Drop the overrun term and you will size the run short by exactly it: the documented 2100s host resolves a 2415s cap, so its real requirement is 2415 + 900 + 285 = 3600s, the **entire** token TTL. Because the clamp guarantees `cap + poll + 285 <= 3600`, a host sitting exactly on that ceiling has **zero** seconds of the hour left for setup — the three-term sum would have told it 3285 and left it believing five minutes of checkout still fit. Three consequences worth knowing before you tune either knob:
...
- **A cap that is above your budget but above the TTL ceiling is narrowed, not rejected and not squeezed** — the host keeps real overrun, just less of it than the number says. That narrowing is reported: an advisory `authoring_hard_cap_clamped` reason names the cap that was asked for, the ceiling it was cut to, and the poll term spending the difference, so the cap the digest reports is the one the run actually used rather than one your config does not contain. It fires whether you wrote the cap in `run.authoring_hard_cap_seconds` or the `* 1.15` default produced it — the message says which — because a silently narrowed default is the same surprise arriving without a config line to explain it. Like the squeeze it is advisory: it describes the host's configuration, not this run's work, so it never marks a run partial and never blocks the auto-merge.
