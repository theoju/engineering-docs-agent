...
- Tests: `tests/state_io/test_add_partial_blind.py` (blind/degraded/info_only precedence, including the monotonic "a degraded run that later goes blind stays blind" case), `tests/orchestrator/test_dispatch_reasons_classification.py` (`_record_dispatch_reasons` passthrough), `tests/orchestrator/test_blind_run_interlocks.py` (the three consumers — exit code, watermark, auto-merge — including the regression case a blind + time-truncated + cursor-backed run used to merge), `tests/orchestrator/test_classification_coverage.py` (fails on any unclassified `add_partial` call site), `tests/orchestrator/test_schema_invalid_soft_fail.py` (a schema-invalid source-collector response is a blind blocking reason, distinct from the generic `source_collector_invalid`)
