...
4. **Reason strings are cause-dependent, deliberately.** The walk now serves two causes — time-budget truncation and held-back PRs — so the reason string returned distinguishes `time_budget_*` from `held_back_*`. The `time_budget_*` family stays byte-identical to its pre-CCE-151 text: `tests/orchestrator/test_time_budget.py` and the deferral-skip tests assert those exact strings, and existing runbooks tell operators to grep for them. Neither family was added to `_MERGE_VETO_REASON_PREFIXES` (still the single `app_token_unavailable` entry), and both sites keep `degraded=True`, so CCE-144's blind/degraded classification is unaffected by this change.
   ...
   This switch on `_rsn` is applied at three of the four `add_partial` call sites inside the cursor-walk branch (`scripts/orchestrator_runner.py:run`) — the no-cursor case, the unanchored-deferred case, and the cursor-unresolvable-in-repo case. The fourth, where a resolved cursor still fails the `_sha_in_window` check, still hardcodes the literal `time_budget_advance_out_of_window` regardless of `_rsn`. A held-back (non-truncated) run that reaches exactly that branch reports a `time_budget_*`-prefixed reason for a run that never touched the time budget. It does not change what the run does — the advance is still refused and the reason is still `degraded=True` — but a grep for `held_back_advance_out_of_window` will not find it. Treat this as a known naming gap, not a behavioral one, until it is closed under its own ticket.
