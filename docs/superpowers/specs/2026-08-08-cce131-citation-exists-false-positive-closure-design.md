...
Add `<docs_dir>/<token>` as a second resolution candidate, reading `docs_dir` from the `site:` config block, and skip tokens whose first path segment is the build-output directory — mkdocs `site_dir`, which the host's mkdocs config may set and which defaults to `site` when a parsed config sets none. Parse `mkdocs.yml` permissively: a plain YAML parser aborts on mkdocs-material's custom tags (`!!python/name:`, `!ENV`), so the parse must tolerate and discard unknown tags rather than fail the whole read on them. When no mkdocs config parses at all — missing file, unparseable YAML, or a host on a different generator entirely — skip nothing. Falling back to a bare `site` default in that case is not a no-op: on a host with no mkdocs config that keeps real source under `site/`, it would reserve that prefix unconditionally and let an invented path under it pass.
