...
That sizing is now **enforced structurally**, not just documented: `resolve_authoring_hard_cap` clamps the hard cap down to `GITHUB_APP_TOKEN_TTL_SECONDS - checks_timeout_seconds - 120s` (the poll term is dropped for a `merge.policy: manual` host, which never runs it), so no host can configure an authoring overrun that outlives its own token. Two consequences worth knowing before you tune either knob:
