...
## Pre-admission window cap

The soft budget below bounds the clock. It does not bound the work, and until CCE-169 nothing did: the review window was a lookback from a baseline that advances slowly, so a stalled baseline widened by a day every night and each run finished a smaller fraction of it. The only truncation was the time-based cut inside the admission loop, which fires after the run has already begun failing to keep up.

`resolve_window_cap` (`scripts/orchestrator_runner.py:resolve_window_cap`) resolves `run.window_pr_cap` in config over `DEFAULT_WINDOW_PR_CAP` — 10 (`scripts/orchestrator_runner.py:DEFAULT_WINDOW_PR_CAP`). `0` is the advertised opt-out and restores the pre-CCE-169 unbounded window. The key is **default-ON**, unlike `citation_source_roots` and `lint.external_repos`: every CCE-169 incident happened on a host that had configured nothing, and an opt-in guard against an unrecoverable failure is discovered by having the failure. A cap set too tight is a visible nightly reason an operator raises in one edit; a cap set too loose is a silent stall.

The cut runs on the oldest-first PR list, before the admission loop. PRs past the cap become `window_capped`, which is a **third category** — not a reuse of `admission_deferred`. The two have the same shape and different causes (`admission_deferred` means the run TRIED and ran out of time; `window_capped` means the run deliberately DID NOT TRY), and per CCE-144 classification follows the call site, never the resemblance. Three consumers, and only the first takes them:

- **`held_back`** — YES. Capped PRs stop the CCE-151 cursor exactly like unfinished ones, so `advance_cursor_list` (`scripts/orchestrator_runner.py:advance_cursor_list`) breaks at the cap boundary and the baseline never walks past a PR the run did not document.
- **`window_prs` → `next_deferral_counts`** — NO. `next_deferral_counts` (`scripts/orchestrator_runner.py:next_deferral_counts`) pops the entry of any in-window PR that is not still-deferred, and a capped PR can never be still-deferred. Leaving it in `window_prs` would DELETE its genuine deferral history every night it waits.
- **`_deferred_all` → `partition_deferrals`** — NO. The CCE-140 deferral-skip hatch (`scripts/orchestrator_runner.py:partition_deferrals`) must never abandon a PR the run never attempted.

A capped run is partial with a `held_back_window_capped: n of m PRs held for a later run (cap c)` reason, `degraded=True` (CCE-144 — the run HELD BACK what it did not process; it did not consume and lose it). It is a plain literal, deliberately not routed through the `time_budget` / `held_back` reason selector, which only discriminates truncated-vs-degraded and would render a cap as `time_budget_window_capped` on a truncated run — factually wrong, since the cap fires before any clock is consulted. Such a run still auto-merges: `held_back` is non-empty, so the advance is cursor-backed, so CCE-140's `partial and not advance_cursor_backed` carve-out permits the merge — which is what promotes `state.json` and lets the next run take the next `c` PRs.

**One exemption: a PR with no `merge_sha` is never capped.** `_order_prs_oldest_first` (`scripts/orchestrator_runner.py:_order_prs_oldest_first`) keys such a PR as `len(order) + 1`, so it sorts last, and the cap takes the tail — whenever the cap fires, an unanchored PR is guaranteed to be in the capped slice. From there it would enter `held_back` but neither writer of `_deferred_all`, so it never reaches `still_deferred` and the `_no_advance_unanchored_deferred` guard — whose entire purpose is to refuse advancing past a PR that cannot be re-anchored — would never see it. The cursor would advance past its real merge commit and the next window, `cursor..HEAD`, would exclude it permanently. The invariant is that **the cap may only hold back a PR a later window can re-anchor**, so unanchored PRs are pulled back out of the capped tail and admitted. The work bound is `cap + unanchored`, not exactly `cap`.
...
The cache is opt-out, not opt-in: `run.reuse_pr_summaries` defaults to `True`, so an existing host gains the savings with no config edit. `next_pr_summaries` (`scripts/orchestrator_runner.py:next_pr_summaries`) writes the persisted `state["pr_summaries"]` map each run, evicting entries after `PR_SUMMARY_RETENTION_DAYS` (30) days of not being seen — by last-seen date, not by window membership, because the window can shrink transiently when source-collector degrades, and wiping the cache at exactly the moment the pipeline is already struggling would be the worst time to lose it. A PR that keeps being deferred (and so never gets a fresh summary) still has its `last_seen_at` refreshed each run, so it never ages out while it is still in play. `pr_summaries` is a schema-declared top-level key (`templates/state.schema.json`), keyed the same way as `deferral_counts` (`{owner}/{name}#{pr}`), with `merge_sha`, `fingerprint`, `last_seen_at`, and the raw `summary` all required on every entry — the schema is what makes a re-stamped `pr_number` legible as "not authoritative" rather than an ad hoc convention.
...
## Deferral-skip stall clock: escaping a counter that can't reach its own threshold

CCE-140's escape hatch counts consecutive deferrals in `state.json:deferral_counts` and abandons a PR once it hits a threshold (default 3), so a permanently-blocked PR can't freeze the cursor forever. But `state.json` only reaches `main` when the docs-agent PR merges, and the PR does not merge while the run is `partial` and not cursor-backed (CCE-140's own gate). A run is partial *because* something is deferred — the exact condition the counter exists to escape — so every night recomputes `count + 1` into a branch that never lands, and the threshold is unreachable by construction. Observed on `theoju/claude-code-self-assessment`: the baseline pinned at `175162e1` for five straight nights, with `deferral_counts` for #235/#236 recomputed to the same value every run because the write never reached `main`.

CCE-175 adds a wall-clock backstop keyed on a signal that is already on `main` and was never gated on merge: `last_successful_run.completed_at`. `baseline_stall_days(state, now=)` (`scripts/orchestrator_runner.py:baseline_stall_days`) returns the baseline's age in days, or `None` when it can't be determined — a bootstrap host with no `last_successful_run`, a missing `completed_at`, or an unparseable timestamp. Absence must never read as an infinite stall, or a bootstrap host's first-ever run would forgive every PR in the window. A `completed_at` in the future (clock skew) clamps to `0.0` rather than going negative.

`resolve_deferral_stall_days(config)` (`scripts/orchestrator_runner.py:resolve_deferral_stall_days`) derives the stall window as `resolve_deferral_threshold(config) + 1`, not a separately-tunable constant, so the clock is by construction always slower than the counter it backs up — raising the threshold moves the window with it for free. `run.deferral_stall_days` overrides the derived value directly; a threshold `<= 0` (the CCE-140 opt-out for disabling skipping entirely) resolves the stall window to `0` too, so a host that has turned the hatch off can't have it reopened by the clock.

When the baseline is older than the stall window, `partition_deferrals` (`scripts/orchestrator_runner.py:partition_deferrals`) takes an explicit `forgive` set of PR numbers and abandons them regardless of their own consecutive-deferral count.

**Only the prefix blocker is forgiven — not every deferred PR in the window.** The first version forgave everything the stall window covered, and it broke on its first live run (35609168489, 2026-09-21): #246 and #249 were deferred for the first time, blocked by an unrelated page, and abandoned immediately just because an earlier PR (#235) had stalled the baseline — deleting the three chances the threshold is supposed to guarantee. The narrower fix people reach for next — forgive only PRs that already have a nonzero count — reopens the same deadlock from a different angle: a first-time deferral sitting at the oldest cursor position blocks the prefix at index 0, so nothing merges, so its count can never reach 1, so it can never qualify to be forgiven either. `test_the_deadlock_breaks_once_the_clock_is_consulted` (`tests/orchestrator/test_deferral_stall_escape.py`) pins exactly this shape and is what caught the count-gated draft before it shipped. What holds: forgive the single oldest deferred PR — the actual cursor blocker — one per stalled run, nothing else.

A forgiven run records an info-only `deferral_stall_escape: baseline has not advanced in Nd (window Md); forgiving the oldest deferred PR #k so state can reach main` reason. It's `info_only` because forgiveness is a deliberate, self-documenting choice landed in `skipped_prs`, not a malfunction — but see the merge-gate fix below, because recording the reason alone was not enough to let the run merge.

## A forgiven run must still count as cursor-backed (CCE-178)

CCE-175's escape fired correctly on its first production run and the PR still did not merge — `auto_merge_skipped: partial_run`. Forgiving the blocker empties `held_back`, which routes the advance through the plain window-HEAD `else` branch described in the cursor-backed section above, and that branch set `advance_cursor_backed = False` unconditionally: until this fix, an empty `held_back` only ever meant "nothing was deferred." CCE-140's merge gate is `partial and not advance_cursor_backed`; a forgiven run is still `partial` (the escape reason doesn't clear that), so the gate kept skipping the merge — the escape had moved the run from one blocked path to another without fixing the thing that actually blocks it.

The fix narrows what `advance_cursor_backed` means on the `else` branch: it is now `bool(skipped_numbers)` rather than a bare `False`. An empty `held_back` is ambiguous between two different runs — nothing was ever deferred, or everything that was deferred got explicitly forgiven — and only a run that recorded a real, auditable skip counts as cursor-backed. A clean run with no forgiveness still reports `False`, unchanged from before.
