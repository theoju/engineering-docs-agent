...
- `scripts/verify_runner.py` carries three further blocking sites (publish-verifier dispatch classified blind; the CCE-63 CircleCI degrade and its notifier classified degraded), audited for the same classification-coverage sweep. It is a separate entry point — this change does not alter its exit code.
- Tests: `tests/state_io/test_add_partial_blind.py` (the `degraded` keyword's precedence), `tests/orchestrator/test_classification_coverage.py` (every blocking call site is explicitly classified, no reliance on the fail-safe default), `tests/orchestrator/test_blind_run_interlocks.py` (the three consumers of the `blind` flag — exit code, watermark advance, auto-merge gate), `tests/orchestrator/test_dispatch_reasons_classification.py`, `tests/orchestrator/test_cursor_backed_merge.py`, `tests/orchestrator/test_deferral_skip.py`, `tests/orchestrator/test_state_advancement_invariant.py`, `tests/orchestrator/test_schema_invalid_soft_fail.py`, `tests/orchestrator/test_pipeline_integration.py` (fixture-driven dry-run coverage of the blind/degraded split end to end)
