...
Source: `docs/adr/0001-baseline-advances-on-disk-for-partial-runs.md`.
...
becomes the next window's lower bound. (See the **baseline**, **cursor**, and
**ephemeral advance** entries in the [domain glossary](../architecture/glossary.md)
if these terms are unfamiliar — they're used near-interchangeably in code
despite naming distinct things.)

CCE-140 later introduced deferral tracking, and with it a contrary intuition —
that a run which owes pages should not compute an advance past the PRs that
owe them. Its clamp was scoped to runs cut short by a time budget, so the two
ideas did not collide in practice: a content-validator block still advanced on
disk, per CCE-40, and the merge gate refused to promote it.
...
advance and reasonably suspect a bug. That reaction is why this record
exists.
