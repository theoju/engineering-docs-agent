...
- Tests: `tests/state_io/test_add_partial_blind.py` pins the three-way `add_partial` precedence (blind-by-default, `degraded=True`, `info_only=True`) directly; `tests/orchestrator/test_classification_coverage.py` walks the AST of `scripts/orchestrator_runner.py` and `scripts/verify_runner.py` and fails on any `add_partial` call site that omits an explicit `info_only`/`degraded` keyword, so the classification can't decay silently as the file grows. `tests/orchestrator/test_blind_run_interlocks.py`, `tests/orchestrator/test_dispatch_reasons_classification.py`, `tests/orchestrator/test_cursor_backed_merge.py`, `tests/orchestrator/test_deferral_skip.py`, `tests/orchestrator/test_pipeline_integration.py`, `tests/orchestrator/test_schema_invalid_soft_fail.py`, and `tests/orchestrator/test_state_advancement_invariant.py` cover the downstream consumers (exit code, watermark, auto-merge) and the specific call-site classifications called out above.
