...
- **Page-path frame for `section_generator_for` (sub-plan 1 handoff).** The precursor's `frontmatter_contract.section_generator_for` matches a page against `docs_dir/section_path` by segment-bounded prefix, so the page path must be in the same frame as `docs_dir` (absolute or repo-relative). A docs_dir-relative or bare path silently yields the _default_ field set — a silent wrong-answer, not an error. This is inert in the precursor (the orchestrator passes absolute paths; the dogfood config has no `site:` block), but **sub-plans 2-3 must** normalize page paths to a documented frame before calling the resolver (or make the match robust to docs_dir-relative paths) **and** add an end-to-end test pinning the orchestrator's real absolute-path frame. Relatedly, `docs_dir` and the `lens_paths` that produce page paths are configured independently — sub-plans 2-3 must ensure they stay mutually consistent or the resolver silently misses.
