...
The first is an ordinary boundary deferral and the run still advances. The second is the bounded forced cut described above. The third fires on a **squeezed** host, where the TTL ceiling lands at or below `time_budget_seconds` itself — the resolver applies no clamp at all in that case (there's no overrun left to bound), records an advisory `authoring_hard_cap_squeezed` reason, and degrades to the pre-CCE-152 mid-group cut. The stock `DEFAULT_TIME_BUDGET_SECONDS` (2700s) lands in exactly this squeezed state, so it's the wording most operators will actually see.
...
Landing the config key itself needed a second fix. `run.authoring_hard_cap_seconds` was documented in the resolver's own docstring and read by `resolve_authoring_hard_cap` while `templates/config.schema.json` still declared the `run` block `additionalProperties: false` without listing the new key — so a host that followed the documentation had its config rejected at load (`scripts/state_io.py:load_config_validated`, exit 2) before the resolver ever ran. Every resolver unit test passed a raw dict straight to the function and never touched the loader, so nothing caught the drift. `tests/schemas/test_config_schema.py` now exercises the schema directly — `authoring_hard_cap_seconds` accepts a positive integer and rejects `0`, a negative value, a string, and a float (the `integer` type also guards the resolver's own `int(val)` cast, which had no other guard against a typo like `"soon"`).

## Verification

`tests/orchestrator/test_pr_boundary_authoring_cut.py` drives the fix end to end: a past-soft-deadline cut mid-group keeps running to the PR boundary and the baseline advances to it; a shared batch between two PRs is recognized as owned by the older one; a hard-cap cut still lands inside the group and reports that the baseline cannot advance; and a skipped (`unknown_lens`) batch doesn't fabricate a spurious boundary. `tests/orchestrator/test_authoring_hard_cap_bounds.py` covers the cap resolver's clamping and squeeze arithmetic against `load_config_validated` — the real host-config loader, not a raw dict — precisely because that gap between resolver and schema was invisible to a resolver-only test. `tests/schemas/test_config_schema.py` pins the schema's own accept/reject boundary for `authoring_hard_cap_seconds` in isolation.
