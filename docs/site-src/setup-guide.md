...
- [ ] Note the App Client ID (shown on the App's General page; starts with `Iv1.` or `Iv23li…`).
...
- **CCE-60**: Consolidate the full setup guide from the unreachable `docs/setup-guide.md` root copy into the published `docs/site-src/setup-guide.md` path (canonical published location).
- **CCE-66**: Migrate `actions/create-github-app-token@v3` from the deprecated `app-id` input to `client-id`; promote `JIRA_EMAIL` from Repository Secret to Repository Variable (email is a basic-auth username, not a credential).
