...
An explicit `authoring_hard_cap_seconds` at or below the resolved budget is rejected as a config error (exit 2) rather than silently clamped up — equal collapses the hard deadline onto the soft one and quietly restores the pre-fix mid-group cut in exactly the place an operator was trying to configure it away. `authoring_hard_cap_seconds` is declared in `templates/config.schema.json` under `run` (`run` carries `additionalProperties: false`, so a misspelled key like `authoring_hardcap_seconds` fails config load rather than being silently ignored); the schema cannot itself express "greater than `time_budget_seconds`" — JSON Schema has no sibling-property comparison — so that comparison is `resolve_authoring_hard_cap`'s job at runtime, not the schema's. A `merge: {policy: manual}` host is not charged for the merge-check poll it never runs, so its resolved ceiling — and hence its overrun — is larger than an otherwise-identical auto-merge host's.

**The stock default has no overrun to give.** `DEFAULT_TIME_BUDGET_SECONDS` is 2700s; add the default 900s merge-check poll and the token's 3600s TTL is spent with nothing left for a tail. On that host the resolver returns the cap held flat at the budget — `authoring_hard_cap == time_budget_seconds` — records an advisory `authoring_hard_cap_squeezed` reason, and behaves exactly like the pre-CCE-152 cut: it may still land mid-group, and a run that does earns no advance, same as always. That is not a config error (nothing is misconfigured — the budget itself is perfectly serviceable, it just leaves no room for a bounded overrun on top) and the resolver does not abort or truncate to compensate; degrading loudly is the whole mitigation for a host in this state. Because the squeezed cap equals the budget, the ordinary hard-cap wording would render as "hard cap 2700s over budget 2700s" — a number over itself — so the squeezed case gets its own phrasing:
...
This is the state most operators actually meet, since it's what the stock default resolves to. A cap that is legal but merely narrowed by the clamp (not squeezed to the budget) gets a third advisory, `authoring_hard_cap_clamped`, naming the resolved cap, the ceiling, and which config value produced it — so the digest never reports a number silently different from what was configured.

## Verification

`tests/orchestrator/test_pr_boundary_authoring_cut.py` drives the fix end to end: a past-soft-deadline cut mid-group keeps running to the PR boundary and the baseline advances to it; a shared batch between two PRs is recognized as owned by the older one; a hard-cap cut still lands inside the group and reports that the baseline cannot advance; a squeezed host's cut is worded so it doesn't read as a number over itself; a manual-merge host is not charged for a checks poll it never runs; a configured `authoring_hard_cap_seconds` loads and governs the cut end to end through the host's own config file; the clamped cap (not the raw ratio) is what actually bounds the running loop; an at-or-below-budget override fails cleanly with exit 2 instead of a traceback; and a skipped (`unknown_lens`) batch doesn't fabricate a spurious boundary. `tests/orchestrator/test_authoring_hard_cap_bounds.py` covers the cap resolver's clamping and squeeze arithmetic in isolation.
