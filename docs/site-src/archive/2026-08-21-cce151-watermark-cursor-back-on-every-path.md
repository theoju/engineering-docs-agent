...
3. **Merge-as-promotion was never the containment it looked like.** The pre-fix assumption was that a human merging the docs PR by hand acts as a safety check. But merging is all-or-nothing: PR #203 on the host carried a recovered page and a poisoned watermark advance in the same diff, and the operator's only options were to accept both or lose the recovery — a gate that cannot be applied to half a PR is not a gate on the half that is wrong.
