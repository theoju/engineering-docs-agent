...
def test_04_literal_equals_shape_contract(template_doc, dogfood_doc) -> None:
    ...
    for doc, label in [(template_doc, "template"), (dogfood_doc, "dogfood")]:
        ...
        # nightly cadence baseline) plus `workflow_dispatch:` (operator
        # manual fire). The dogfood schedule was briefly paused in PR #108
        # while D1 + D2 were authored; PRs #112 + #113 + this PR restored
        # parity. If a future operational pause is needed, splitting this
        # assertion template-only is the precedent (see PR #108).
