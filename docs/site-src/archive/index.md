...
- **CCE-180: page-author's schema rejected its own documented failure shape** — `schema_invalid: page-author: None is not of type 'string'` fired on 5 of the
- **CCE-181: Cross-Repo Citations — Declared External Repos, Rendered Before Lint (2026-09-21)** — `page-author` sometimes has to point at a file that lives in a **different repository** than the one it is documenting. On 2026-09-21 it was documenting plugin work on a host repo and wrote the plugin's own path as an ordinary citation:
- **CCE-141: automatic citation repair was built, then withdrawn** — `page-author` sometimes shortens an already-resolvable citation. The
...
- **CCE-144: Blind-Run Detection** — **Date:** 2026-08-14
- **CCE-148: The Extraction Prompt, Not Batching, Was Graphify's Doc-Layer Ceiling** — Four prior extraction passes over `docs/superpowers/{specs,plans}` all landed
- **CCE-150: the blind-run-detection abandon call, reversed nine minutes later** — > **This page is a decision-history record, not a description of current behavior.** If you want the actual, currently-shipped design for blind-run detection, read the CCE-144 entry in `CLAUDE.md` and its spec at `docs/superpowers/specs/2026-08-13-cce144-blind-run-detection-design.md`. Nothing below describes what the runner does today.
- **Graphify Semantic-Extraction Findings (2026-08-13)** — **PR:** #216
- **Reference-aware batching: rejected (2026-08-13)** — Reference-aware batching — dispatching each spec or plan to the extraction
...
- **Plans archive** — _Auto-generated; 79 entries. Do not edit by hand — see `scripts/archive_indexes.py`._
